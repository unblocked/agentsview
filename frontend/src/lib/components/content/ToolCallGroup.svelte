<script lang="ts">
  ...
  let hasUnblocked = $derived(
    toolSegments.some(
      (s) => s.toolCall?.tool_name?.startsWith("mcp__unblocked__"),
    ),
  );
...
<div class="tool-group" class:has-unblocked={hasUnblocked}>
  <div class="tool-group-header">
    {#if hasUnblocked}
      <span class="unblocked-icon">
...
<style>
  ...
  .tool-group-body :global(.tool-block.unblocked) {
